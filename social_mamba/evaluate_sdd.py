...
def evaluate_ade_fde(model, dataloader, bs, config, logger, return_all=False, bar_prefix="", per_joint=False, show_avg=False):
    ...
    # read scale when available; some rebuttal/checkpoint trees only contain normalized SDD caches.
    scale_path = 'data/SDD_meter2pixel_scale.npy'
    scale = np.load(scale_path) if os.path.exists(scale_path) else 1.0
    ...
    for i, batch in enumerate(dataloader):
        ...
        for k in range(len(out_joints)):
            ...
            ADE.append(norm.item())
            FDE.append(scene_fde.item())
    ...
    # rescale to pixel
    ADE_pixel = np.array(ADE) * scale
    FDE_pixel = np.array(FDE) * scale
    ade_pixel = ADE_pixel.mean()
    fde_pixel = FDE_pixel.mean()

    return ade, fde, ade_pixel, fde_pixel, num_test_samples
...
if __name__ == "__main__":
    ...
    ade,fde, ade_pixel, fde_pixel, num_samples = evaluate_ade_fde(model, dataloader, bs, config, logger, return_all=True)

    print('ADE: ', ade*rescale)
    print('FDE: ', fde*rescale)
    print('ADE(pixel)', ade_pixel)
    print('FDE(pixel)', fde_pixel)
