...
def cache_root():
    return Path(os.environ.get("SOCIAL_MAMBA_CACHE_DIR", Path(__file__).resolve().parents[1] / "data" / "cache"))
...
class MultiPersonTrajPoseDataset(torch.utils.data.Dataset):
    ...
    def load_data(self):
        ...
        for data_name in data_names:
            directory = cache_root() / data_name / split
            if not directory.exists():
                raise FileNotFoundError(
                    f"Cache directory not found: {directory}. Set SOCIAL_MAMBA_CACHE_DIR or prepare data/cache."
                )
            for filename in sorted(os.listdir(directory)):
                path = directory / filename
                if path.is_file():
                    self.datalist.append(str(path))
