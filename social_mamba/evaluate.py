...
def evaluate_ade_fde(model, dataloader, bs, config, logger, return_all=False, bar_prefix="", per_joint=False, show_avg=False):
    ...
    for i, batch in enumerate(dataloader):
        ...
        in_joints = in_joints.to(config["DEVICE"])
        padding_mask = padding_mask.to(config["DEVICE"])
        ...
        in_joints, in_masks, out_joints, out_masks, padding_mask = sort_social_grid(
            ...
            strategy=config.get("MODEL", {}).get("social_grid_type", "none")
        )
...
def evaluate_ade_fde_diversity(model, dataloader, bs, config, logger, return_all=False, bar_prefix="", per_joint=False, show_avg=False):
    in_F, out_F = 10, 20
    bar = Bar(f"EVAL ADE_FDE", fill="#", max=len(dataloader))
    ...
    for i, batch in enumerate(dataloader):
        ...
        # TODO: diversity metrics: pairwise distance between K predictions
        ...
        for k in range(len(out_joints)):
            ...
            # TODO: diversity metrics: pairwise distance between K predictions
            pred_xy_for_div = pred_joints[k,:,:,:2] # Shape (K, out_F, 2)
            temp_K_div = pred_xy_for_div.shape[0]

            if temp_K_div > 1:
                # Get final endpoints of all K predictions for this sample
                final_points = pred_xy_for_div[:, -1, :]  # Shape (K, 2)

                # Calculate pairwise distance matrix (K, K)
                # Use broadcasting: (K, 1, 2) - (1, K, 2) -> (K, K, 2)
                p1 = final_points.unsqueeze(1)
                p2 = final_points.unsqueeze(0)
                diff = p1 - p2
                dist_matrix = torch.norm(diff, p=2, dim=-1) # Shape (K, K)

                # Sum the upper triangle of the matrix (excluding the diagonal)
                # which contains all unique pairwise distances
                total_pairwise_dist = torch.sum(torch.triu(dist_matrix, diagonal=1))

                # Calculate the number of unique pairs
                num_pairs = (temp_K_div * (temp_K_div - 1)) / 2.0

                # Average pairwise distance for this scene
                scene_avg_div = total_pairwise_dist / num_pairs

                div_batch += scene_avg_div
    ...
    ade = ade_batch/num_test_samples
    fde = fde_batch/num_test_samples
    div = div_batch / num_test_samples  # <--- ADD THIS LINE
    return ade, fde, div, num_test_samples
...
if __name__ == "__main__":
    ...
    rescale = (1/0.3048)*(28/94) if name[0] == "NBA" else 1.0
    ...
    # ade,fde,div,num_samples = evaluate_ade_fde_diversity(model, dataloader, bs, config, logger, return_all=True)
